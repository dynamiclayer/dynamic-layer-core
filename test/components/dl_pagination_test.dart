...
void main() {
  ...
  testWidgets('renders count items with 12x12 boxes and 8x8 centered dots', (
    ...
    await pumpPagination(tester, pagination: const DlPagination(count: 4));
    ...
    expect(
      find.byWidgetPredicate(
        (widget) => widget is SizedBox && widget.width == DlSpacingTokens.p_4,
      ),
      findsNWidgets(3),
    );
  });
  ...
  testWidgets(
    'selected dot is black and unselected dots are black 40% opacity',
    (tester) async {
      ...
      BoxDecoration dotDecoration(int index) =>
          tester
                  .widget<Container>(
                    find.byKey(Key('dl_pagination_dot_$index')),
                  )
                  .decoration
              as BoxDecoration;
  ...
  testWidgets('supports custom selected and unselected dot colors', (
    tester,
  ) async {
    await pumpPagination(
      tester,
      pagination: const DlPagination(
        count: 3,
        selectedDotColor: Colors.white,
        unselectedDotColor: Color(0x66FFFFFF),
      ),
    );
    ...
    expect(dotDecoration(1).color, const Color(0x66FFFFFF));
    expect(dotDecoration(2).color, const Color(0x66FFFFFF));
  });
}
